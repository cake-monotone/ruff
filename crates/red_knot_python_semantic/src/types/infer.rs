...
impl<'db> TypeInferenceBuilder<'db> {
            ...
            (Type::Tuple(lhs), Type::Tuple(rhs)) => {
                ...
                match op {
                    ...
                    ast::CmpOp::In | ast::CmpOp::NotIn => {
                        ...
                        // Note: It’s not possible to definitively determine if an element is "definitely not in" the tuple.
                        // ex) `tuple[int] in tuple[tuple[int]]`
    ...
    /// To facilitate lexicographic ordering, elements from both slices are compared pairwise using
    /// `infer_binary_type_comparison`. If a definitive result cannot be determined as a `BoolLiteral`,
    /// `bool` is returned. If the comparison is undefined between types, it returns `None`.
    fn infer_lexicographic_type_comparison(
        ...
        for (l_ty, r_ty) in left.iter().zip(right.iter()) {
            ...
            match eq_result_ty {
                // Elements are equal, continue to the next pair
                Some(Type::BooleanLiteral(true)) => continue,

                // Elements are not equal, perform the specified comparison and return the result
                Some(Type::BooleanLiteral(false)) => {
