...
/// ## What it does
/// Checks for unnecessary type defaults.
///
/// ## Why is this bad?
/// Python 3.13 introduced a new feature: type defaults for type parameters.
/// Now, you can omit default type parameters for some types in the standard library (e.g., Generator, AsyncGenerator).
/// Including unnecessary default type parameters can make the code more verbose and less readable.
///
/// ## Examples
/// ```python
...
#[violation]
pub struct UnnecessaryTypeDefault;

impl AlwaysFixableViolation for UnnecessaryTypeDefault {
    #[derive_message_formats]
    fn message(&self) -> String {
        format!("Unnecessary type defaults")
    }

    fn fix_title(&self) -> String {
        format!("Remove unnecessary type defaults")
    }
}

// RUF031
pub(crate) fn unnecessary_type_default(checker: &mut Checker, expr: &Expr) {
    if let Expr::Subscript(ast::ExprSubscript { value, slice, .. }) = expr {
        ...
        let mut diagnostic = Diagnostic::new(UnnecessaryTypeDefault, expr.range());
